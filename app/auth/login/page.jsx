...
export default function LoginPage({ searchParams }) {
  // Normalize searchParams values (Next may give array|string)
  const mockFlag = firstValue(searchParams?.mock) === "1";
  const token = firstValue(searchParams?.token) ?? null;

  return (
    <main>
      {/* Suspense is optional here because the client component doesn't use useSearchParams.
          It's left in case you want to lazy-load or keep a fallback UX */}
      <Suspense
        fallback={
      ...
      >
        <LoginClient mock={mockFlag} token={token} />
      </Suspense>
    </main>
  );
}
