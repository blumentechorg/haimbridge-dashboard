...
export default function LoginClient({
  ...
  useEffect(() => {
    // If server passed ?mock=1, auto-login client-side (set token then redirect)
    if (mock) {
    ...
    // If server passed a token via ?token=xxx, save and redirect
    if (tokenFromQuery) {
      saveToken(tokenFromQuery);
      router.push("/dashboard");
    }
  }, [mock, tokenFromQuery, router]);
